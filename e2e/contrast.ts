 ...
 *    files the node under `incomplete`. `body` here paints two large radial
 *    washes fading to transparent, `.exhibit` and `.panel .pb` each sit on a
 *    `linear-gradient` overlay pair, and `.cl-hero-why` is a
 *    `color-mix(in oklab, var(--acc) 6%, transparent)`. Almost nothing on this
 *    page composites onto a flat colour.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. `.cl-hero-sub` renders
 *    `--tx` at `opacity: .85`, and `.quiz-opt:disabled` — every option once its
 *    question is answered — drops to `0.9`.
 *
 ...
 *    tables sit in `.table-wrap` scrollers and the sticky `.toc` is a
 *    horizontal chip scroller, so at 380px much of both is scrolled out of the
 *    frame. Content past the client box is not dimmed or partly drawn — it is
 ...
 *    draws its ceremony chain and circuit wires as HTML, not SVG, so the rule
 *    is silent here — but it is kept because it is cheap: where an `<svg>` does
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * matters" wash is `color-mix(in oklab, ...)` and the sticky nav's
     * backdrop is `color-mix(in srgb, ...)`. Chromium reports both of those
     ...
     * the farthest-corner radius. That matters a lot here: `body` paints two
     * 1200x700 radial washes anchored off the TOP corners and fading to
     * transparent by ~60%, so judging the quiz at the bottom of a very long
     * page by the corner colour would invent a backdrop nothing down there
     * sits on. A non-gradient layer (`url()`, `none`) is unmeasurable and
     * paints nothing.
     */
     ...
     *
     * This is one very long single-page document — six exhibits, a glossary, a
     * five-question quiz and several comparison tables — and every text node
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page uses no native `<details>` — its result
      // regions are `[hidden]`-toggled instead — so the check is silent here
      // and kept as a cheap guard.
     ...
     * it, the `aria-hidden` decorations on this page — the `.rp-spinner` and
     * the icon glyphs in the exhibit badges — are measured as if they took the
     * element's declared `color`, which describes nothing on screen.
     */
     ...
     * lab authors no SVG at all, so the rule is silent here.
     */
